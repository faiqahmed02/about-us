(function ($) {
    ...
    if ($('.premium-floating-effects-yes').length) {

        var isEdit = "undefined" !== typeof elementorFrontend ? elementorFrontend.isEditMode() : false;

        if (isEdit) {

            if ('widget' === window.current_scope.data('element_type'))
                premiumFloatingEffectsHandler(window.current_scope);

        } else if (window.scopes_array) {
            Object.values(window.scopes_array).forEach(
                function ($scope) {
                    if ('widget' === $scope.data('element_type'))
                        premiumFloatingEffectsHandler($scope);
    ...
    function premiumFloatingEffectsHandler($scope) {
