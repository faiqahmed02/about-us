...
class Admin_Notices {
	...
	/**
	 * Checks if New Features message is dismissed.
	 *
	 * @since 4.7.0
	 * @access public
	 *
	 * @return void
	 */
	public function handle_nf_notice() {

		if ( ! isset( $_GET['new_features'] ) ) {
			return;
		}

		if ( 'opt_out' === $_GET['new_features'] ) {
			check_admin_referer( 'opt_out' );

			update_option( 'new_features_notice', '1' );
		}

		wp_redirect( remove_query_arg( 'new_features' ) );
		exit;
	}
	...
	/**
	 * Shows admin notice for New Features.
	 *
	 * @since 4.7.0
	 * @access public
	 *
	 * @return void
	 */
	public function get_nf_notice() {

		$nf_notice = get_option( 'new_features_notice' );

		if ( '1' === $nf_notice ) {
			return;
		}

		$dc_url = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/elementor-display-conditions/', 'bf21-update-notification', 'wp-dash', 'bf21-update' );

		$fe_url = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/elementor-floating-effects-animation/', 'bf21-update-notification', 'wp-dash', 'bf21-update' );

		$bg_url = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/elementor-animated-blob-generator/', 'bf21-update-notification', 'wp-dash', 'bf21-update' );

		$wc_url = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/elementor-animated-blob-generator/', 'bf21-update-notification', 'wp-dash', 'bf21-update' );

		$global_url = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/premium-addons-global-features-for-elementor/', 'bf21-update-notification', 'wp-dash', 'bf21-update' );
		...
		<div class="error pa-notice-wrap pa-new-feature-notice">
			...
			<div class="pa-text-wrap">
				<p>
					<?php
					echo __( 'Major update for both Free & PRO versions. <b>NEW</b>: ', 'premium-addons-for-elementor' ) . '<a href="' . $dc_url . '" target="_blank">Display Conditions,</a><a href="' . $fe_url . '" target="_blank">Floating Effects,</a><a href="' . $bg_url . '" target="_blank">Blog Generator</a> and<a href="' . $wc_url . '" target="_blank">WooCommerce Listing.</a> Check also<a href="' . $global_url . '" target="_blank">Global Features</a> page.';

					?>
				</p>
			</div>
			<div class="pa-notice-close" data-notice="new-features">
